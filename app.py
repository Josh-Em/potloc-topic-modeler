...
class TopicModel:
    ...
    def train_model(self):
        ...
        try:
            ...
            # Save the model and dictionary to their respective folders
            self.lda_model.save('saved_model/saved_model')
            self.dictionary.save('saved_dictionary/saved_dictionary')
    ...
    def name_topics(self, topics):
        ...
        # Save the named_topics dictionary to a JSON file
        self.save_named_topics()

        return self.named_topics

    # Function to save the named_topics to a JSON file
    def save_named_topics(self):
        try:
            with open('saved_named_topics/saved_named_topics.json', 'w') as f:
                json.dump(self.named_topics, f)
            return True
        except Exception as e:
            logging.error(f"Error saving named_topics: {str(e)}")
            return False
    ...
    # Function to load the saved lda_model, dictionary, and named_topics from their respective files
    def load_saved_content(self):
        try:
            self.lda_model = models.ldamodel.LdaModel.load("saved_model/saved_model")
            self.dictionary = corpora.Dictionary.load("saved_dictionary/saved_dictionary")
            
            with open("saved_named_topics/saved_named_topics.json", "r") as f:
                self.named_topics = json.load(f)
                
            return {"message": "Successfully loaded the saved model, dictionary, and named_topics"}
            
        except Exception as e:
            logging.error(f"Error loading saved model, dictionary, and named_topics: {str(e)}")
            return {"message": f"Error loading saved model, dictionary, and named_topics: {str(e)}"}, 500
...
# Route for loading the saved model, dictionary, and named_topics
@app.route("/load_saved_model", methods=["POST"])
def load_saved_model():
    try:
        result = topic_modeler.load_saved_content()
        return result
    except Exception as e:
        logging.error(f"Error loading saved model, dictionary, and named_topics: {str(e)}")
        return {"message": f"Error loading saved model, dictionary, and named_topics: {str(e)}"}, 500
